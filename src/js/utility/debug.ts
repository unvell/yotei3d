...
// Optional global build-version banner (injected by the build, absent in dev).
...
export class Debugger {
  ...
  showObjectBoundingBox: boolean;

  lastRaycastElapsedTime: number;
  totalFrameRenderingTime: number;
  averageFrameRenderingTime: number;
  lightSourceFilterElapsedTime: number;
  navmeshCheckElapsedTime: number;
  currentLightCount: number;
  totalMeshMemoryUsed: number;
  totalNumberOfTexturesUsed: number;
  totalNumberOfPolygonBound: number;
  numberOfRenderPassesPerFrame: number;
  numberOfSceneRendered: number;
  numberOfPolygonRenderedPerSecond: number;
  _numberOfPolygonRenderedPerSecond: number;
  shadowRenderCount: number;

  totalNumberOfObjectRendered = 0;
  totalNumberOfPolygonRendered = 0;

  _maxPathLength: number;
  ...
  fpsMonitor: {
    ...
    minFPS: number;
    fpsRecord: number[];
    maxFPSNumberOfRecord: number;
    indicatorWidth: number;
    indicatorHeight: number;
  };

  elapsedTime: {
    drawFrameBegin: number;
    drawFrameEnd: number;
    raycastBegin: number | null;
    raycastEnd: number | null;
    lightFilterBegin: number | null;
    lightFilterEnd: number | null;
    navmeshCheckBegin: number | null;
    navmeshCheckEnd: number | null;
  };

  debugPanel: HTMLDivElement;
  fpsCanvas: HTMLCanvasElement;
  objInfoPanel: HTMLDivElement;
  ...
  constructor(renderer: any) {
    ...
    (objInfoPanel.style as any)["text-shadow"] = "0px 0px 1px #ffffff";
  ...
  beforeObjectRender(obj: any): void {

  }

  afterObjectRender(obj: any): void {
    this.totalNumberOfObjectRendered++;
  }

  beforeMeshRender(mesh: any): void {
  }

  afterMeshRender(mesh: any): void {
  ...
  renderDebugInfo(): string {
    const fm = this.fpsMonitor;

    return this.generateDebugInfo("<br/>");
  }

  generateDebugInfo(newline: string): string {
  ...
  showObjectInfoPanel(obj: any): void {
  ...
  hideObjectInfoPanel(): void {
    if (this.objInfoPanel) this.objInfoPanel.style.display = "none";
  }

  getShorterUrl(url: string): string {
    return (url.length > this._maxPathLength) ? ("..." + url.substr(url.length - this._maxPathLength, this._maxPathLength)) : url;
  }

  static get status(): string | undefined {
    const self = this as any;
    if (self.currentScene && self.currentScene.renderer && self.currentScene.renderer.debugger) {
      return self.currentScene.renderer.debugger.generateDebugInfo("\n");
    }
  }

  static dumpCameraPose(): string | undefined {
    const scene = (this as any).currentScene;
    if (scene) {
