...
export class Mesh {
	...
	loadFromStream(stream) {
		...
		if (header[0] === 0x6873656d /* tag: mesh */) {
			...
			if (ver >= 0x0102) {
				...
				if ((flags & Mesh.HeaderFlags.HasNormal) === Mesh.HeaderFlags.HasNormal) {
					this.meta.normalCount = this.meta.vertexCount;
				}
			
				if ((flags & Mesh.HeaderFlags.HasTexcoord) === Mesh.HeaderFlags.HasTexcoord) {
					this.meta.texcoordCount = this.meta.vertexCount;
				}

				if (ver >= 0x0103) {
					const chunkIdBuffer = new Uint32Array(stream, 80, 4);
					if ((flags & Mesh.HeaderFlags.HasLightmap) === Mesh.HeaderFlags.HasLightmap) {
						this._lightmapTrunkId = chunkIdBuffer[0];
						this._lightmapType = header[21];
					}

					if (ver >= 0x0104) {
						if ((flags & Mesh.HeaderFlags.HasGrabBoundary) === Mesh.HeaderFlags.hasGrabBoundary) {
						...
						if ((flags & Mesh.HeaderFlags.HasWireframe) === Mesh.HeaderFlags.HasWireframe) {
							this.meta.edgeCount = header[6];
						}

						if (ver >= 0x0105) {
							if ((flags & Mesh.HeaderFlags.HasRefmap) === Mesh.HeaderFlags.HasRefmap) {
			...
			if ((flags & Mesh.HeaderFlags.HasBoundingBox) === Mesh.HeaderFlags.HasBoundingBox) {
			...
			if ((flags & Mesh.HeaderFlags.HasTangentBasisData) === Mesh.HeaderFlags.HasTangentBasisData) {
				this.meta.tangentBasisCount = this.meta.vertexCount;
			}

			if ((flags & Mesh.HeaderFlags.HasColor) === Mesh.HeaderFlags.HasColor) {
	...
	initMeta(renderer) {
		...
		if (typeof meta.structureType === "number"
			&& meta.structureType === Mesh.StructureTypes.StructureArray) {
    ...
    // skin
		...
		if (renderer.debugger) {
			...
			switch (this.composeMode) {
				default:
				case Mesh.ComposeModes.Triangles:
					this._polygonCount = this.meta.vertexCount / 3;
					break;

				case Mesh.ComposeModes.TriangleStrip:
					this._polygonCount = this.meta.vertexCount - 2;
					break;

				case Mesh.ComposeModes.TriangleFan:
					this._polygonCount = this.meta.vertexCount - 2;
					break;
				
				case Mesh.ComposeModes.Points:
	...
	draw(renderer) {
		...
		if (renderer.currentShader instanceof WireframeShader && meta.edgeCount > 0) {
			...
			switch (this.composeMode) {
				case Mesh.ComposeModes.Points: glPrimitiveMode = gl.POINTS; break;
				...
				case Mesh.ComposeModes.TriangleStrip: glPrimitiveMode = gl.TRIANGLE_STRIP; break;
				case Mesh.ComposeModes.TriangleFan: glPrimitiveMode = gl.TRIANGLE_FAN; break;
			}
	...
	extractPoints() {
		...
		const points = new Array(this.meta.vertexCount)

		for (let i = 0; i < this.meta.vertexCount; i++) {
			const index = i * 3;
			points[i] = new Vec3(this.vertexBuffer[index + 0],
  ...
  hitTestByRay(ray, maxt, session, options) {
		"use strict";
...
export class ParticleMesh extends DynamicMesh {
	constructor(count = 100) {
		...
		this.composeMode = Mesh.ComposeModes.Points;
