...
interface LightningOptions {
  ...
  minInterval: number;
  maxInterval: number;
  onStrike: ((flash: LightningFlash) => void) | null;
}
...
export class LightningFlash {
  ...
  private _baseSun: number[];
  private _baseSky: number[];
  private _lastTime: number;
  private _wait: number;
  private _spikes: Spike[] | null;
  private _elapsed: number;
  private _duration: number;

  constructor(scene: any, options: Partial<LightningOptions> = {}) {
    this.scene = scene;
  ...
  _rand(a: number, b: number): number { return a + Math.random() * (b - a); }

  _readColor(c: any, fallback: number[]): number[] {
